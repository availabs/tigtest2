...
def load_month_rds_data(year, month, source_file, view, column_names, ref_column_names, value_column_configs)
  ...
  SpeedFact.from_partition(year, month).where(view: view).delete_all # delete all previous data
  ...
  begin
    CSV.foreach(source_file, headers: true, return_headers: false,
      ...
      ActiveRecord::Base.transaction do
        ...
        SpeedFact.create_many(fact_rows)
      end
    end
  rescue => e 
    puts e.message
...
if rds_year.blank?
  ...
  rds_month_files = []
  months_to_load = ENV['months'] || ''
  months_to_load = (1..12).to_a.join(',') if months_to_load == 'all'
