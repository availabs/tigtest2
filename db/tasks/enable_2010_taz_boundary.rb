version = '2010'
#seed 2010 TAZ MapBox URL to MapLayers
...
# seed 2010 TAZ area geometries
require 'json'
require 'rgeo/geo_json'

# remove existing 2010 TAZ areas and associated view data
DemographicFact.joins(:area).where(areas: {type: :taz, year: 2010}).delete_all
Area.where(type: :taz, year: 2010).delete_all
...
def load_geom_from_geojson(geom_config)
  ...
  features.each do |feature|
    ...
    next if name.blank?
    area = Area.where(name: name, type: :taz, year: 2010).first_or_create
...
taz_config = {
  file_name: File.join(Rails.root, 'public', 'data', 'NYBPM_TAZ_2010.geojson'),
  match_column: 'TAZ_ID',
