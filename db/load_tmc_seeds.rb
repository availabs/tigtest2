# This seeding script is only executed when ENV['load_tmc'] = true
# A full sample command
#   rake db:seed load_tmc=true
...
def load_tmc_from_geojson(config)
  ...
  features.each do |feature|
    ...
    tmc = Tmc.where(name: name).first_or_create
...
# configuration
tmc_config = {
  file_name: File.join(Rails.root, 'public', 'data', 'NHS_TMC_2013.geojson'),
  match_column: 'TMC'
}
