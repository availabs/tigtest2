...
var yadcf = (function ($) {
	...
	function rangeNumberKeyUP(table_selector_jq_friendly, event) {
		...
		keyUp = function () {
		    ...
		    if (options.ignore_char !== undefined) {
			min = min.replace(options.ignore_char, "");
			max = max.replace(options.ignore_char, "");
		    }
			min = (min !== "") ? (+min) : min;
			max = (max !== "") ? (+max) : max;
					...
					for (col_num in yadcf.getOptions(settings.oInstance.selector)) {
						if (yadcf.getOptions(settings.oInstance.selector).hasOwnProperty(col_num)) {
							if (json['yadcf_data_' + col_num] !== undefined) {
								column_number_filter = col_num;
							  if ((settings.oSavedState !== null	&& settings.oSavedState.ColReorder !== undefined) || (plugins[table_selector_jq_friendly] !== undefined && plugins[table_selector_jq_friendly].ColReorder !== undefined)) {
									column_number_filter = plugins[table_selector_jq_friendly].ColReorder[col_num];
								}
